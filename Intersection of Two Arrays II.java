...
class Solution {
    ...
    private int binarySearch (int [] nums, int tar, int low, int high){
        ...
        while(low <= high){
            ...
            if(nums[mid] == tar){
                //check the first index by either checking if the mid equals low or if the left most element we are looking at is greater then its previous indicating it is the last element of its kind
